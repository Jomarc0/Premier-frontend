...
const GPS_TIMEOUT_MS   = 15000;  // ✅ Increased from 10s to 15s
const GPS_MAX_AGE_MS   = 5000;   // ✅ Increased from 3s to 5s
...
/**
 * FIXED useGpsTracking - Handles timeout #3
 */
const useGpsTracking = (plateNumber, shiftId) => {
    ...
    const sendLocation = useCallback(async (coords) => {
        ...
        try {
            ...
            const res = await driverAPI.post('/location', {
                ...
                recordedAt: new Date().toISOString()  // ✅ Backend expects this
            }, {
    ...
    useEffect(() => {
        ...
        // ✅ FIXED: Better geolocation options
        const options = {
            enableHighAccuracy: true,
            timeout: GPS_TIMEOUT_MS,      // 15s
            maximumAge: GPS_MAX_AGE_MS    // 5s cache
        };
